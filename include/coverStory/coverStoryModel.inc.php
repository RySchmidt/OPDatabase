...
function deleteCoverStory(object $pdo, int $chapter_number) {	
	$result = selectCoverStoryFromChapterNumber($pdo, $chapter_number);
	$info_cache_id = $result["_cover_story_info_cache_id"];
...
function selectCoverStoryFromTitle(object $pdo, string $cover_story_title) {
	updateViewInfoCacheCoverStory($pdo);
	$query = "SELECT *
		FROM _info_cache_cover_story
		WHERE _info_cache_cover_story.cover_story_title = :title;";
...
function selectCoverStoryFromChapterNumber(object $pdo, int $chapter_number) {
	updateViewInfoCacheCoverStory($pdo);
	$query = "SELECT *
		FROM _info_cache_cover_story
		WHERE _info_cache_cover_story._chapter_number = :chapter_number;";
...
function updateViewInfoCacheCoverStory(object $pdo) {
	$query = "CREATE OR REPLACE VIEW _info_cache_cover_story AS
		...
		ORDER BY _cover_story._chapter_number ASC;";
