...
$original_epithet = $_POST["original_epithet"];
...
try {
	...
	if ($character_id <= 0) {	
		...
		if (empty($character_epithet) 
			|| $info_cache_reveal <= 0) {	
			$character_errors["empty_input"] = "Required field (Epithet, Introduced In) cannot be empty";	
		}
	}
	...
	if ($character_errors) {
		...
		$queryData = [
			...
			"info_type" => 1,
			"character_epithet" => $original_epithet,
	...
	modifyEpithet($pdo, $original_epithet, $character_epithet, $character_id, $original_info_cache, $info_cache_reveal);
