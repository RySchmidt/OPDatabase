...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	
	...
	$chapter_info_cache_id = intval($_POST["original_chapter_info_cache_id"]);
	...
	try {
		...
		if (isChapterInfoCacheIdEmpty($chapter_info_cache_id)) {
			$chapter_errors["id_invalid"] = " Select a chapter to modify.";
		}

		if (chapterIsEmpty($chapter_number, $chapter_title, $chapter_publish_date)) {
		...
		if ($chapter_errors || $cover_story_errors) {
			$_SESSION["modify_A_chapter_errors"] = $chapter_errors;
			$_SESSION["modify_A_cover_story_errors"] = $cover_story_errors;
