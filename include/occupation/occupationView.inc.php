...
function occupationSelection(string $query_data, string $query_name) {
	echo "<select class='form' name='" . $query_name . "'>";
	echo "<option value='-1'> Select Occupation Type </option>";

	try {
		require "dbh.inc.php";
		$results = getAllOccupationFromCharacterId($pdo, $_SESSION[$query_data . "_query_data"]["character_id"]);
		if(!empty($results)) {
...
/*
function occupationInputDisplay() {
